...
@doc raw"""
    rank_ff_abstract(M::MatrixElem{T}) where {T <: NumFieldElem} -> Int
...
@doc raw"""
    rank_ff_abstract(M::MatrixElem{T}) where {T <: FieldElem} -> Int
...
@doc raw"""
    rank_ff_abstract(M::MatrixElem{T}) where {T <: PolyRingElem} -> Int
...
function rank_ff_abstract(M::MatrixElem{T}) where {T <: PolyRingElem}
    ...
    eval_set = get_eval_set(K, det_deg+1)
...
@doc raw"""
    rank_ff_abstract(M::MatrixElem{T}) where {T <: AbstractAlgebra.Generic.RationalFunctionFieldElem} -> Int
...
@doc raw"""
    rank_ff_abstract(M::MatrixElem{T}) where {T <: MPolyRingElem} -> Int
...
function rank_ff_abstract(M::MatrixElem{T}) where {T <: MPolyRingElem}
    ...
    for i = 1:num_vars
        eval_set[i] = get_eval_set(K, det_deg[i]+1)
        if is_empty(eval_set[i])
            #function get_eval_set returns an empty set if and only if K is a finite field and order(K) < det_deg+1 holds. 
            #In this case a field extension L of K such that order(L) >= det_deg+1 is constructed.
            #d is the smallest natural number such that order(K)^d >= det_deg+1.
            d = ceil(Int, log(order(K), ZZ(maximum(det_deg)+1)))
            L, l = Oscar.Nemo._extension(K, d)
...
@doc raw"""
    get_eval_set(K::Field, n::Int) -> Vector{FieldElem}
...
function lift_polynomial(f::MPolyRingElem, Fx::MPolyRing, l::Nemo.FinFieldMorphism{FqField, FqField})
    ...
    g = zero(Fx)
    x = gens(Fx)
    for tup in Oscar.Hecke.cartesian_product_iterator([0:degs[i] for i in 1:num_vars])
        monom = l(coeff(f, tup))
...
function lift_polynomial(f::PolyRingElem, Fx::PolyRing, l::Nemo.FinFieldMorphism{FqField, FqField})
...
#GRAVEYARD FOR OLD USELESS CODE
...
# =========== DET FUNCTIONS WHICH ARE ALMOST IDENTICAL TO OSCAR IMPLEMENTATION OR WORSE ===============

#=
@doc raw"""
    det_ff(A::AbstractAlgebra.Generic.MatSpaceElem{FqPolyRingElem}) -> FqPolyRingElem

Returns the determinant of A using interpolation. 
"""
...
# =============== OLD INVERSE FUNCTION WHICH IS NOT ABSTRACT =====================
...
# =============== OLD KERNEL FUNCTIONS WHICH IS NOT ABSTRACT =====================
...
# OLD VERSION OF GET EVAL SET AND LIFT POLYNOMIAL
...
# UNUSED FUNCTIONS PREIMAGE EVAL - DONT KNOW WHAT THEY WERE FOR
